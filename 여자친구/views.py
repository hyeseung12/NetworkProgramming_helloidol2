...
class CharacterListView(ListView):
    ...
    # 모델_list.html에 모델_list라는 python manage.py makemigrations키로 DB에서 가져온 데이터 넣어서 render 하자
...
class CharacterUpdateView(UpdateView):
    ...
    success_url = reverse_lazy('여자친구:character_list')  # 수정 완료 후 이동할 URL

class CharacterDeleteView(DeleteView):
    model = Character
    success_url = reverse_lazy('여자친구:character_list')  #삭제 완료 후 이동할 URLURL